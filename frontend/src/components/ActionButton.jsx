...
const ActionButton = ({
    ...
    const variants = {
        primary: 'bg-[#FDB4B7] hover:bg-[#F2C6C2] text-white border border-transparent',
        secondary: 'bg-gray-200 hover:bg-gray-300 text-gray-800 border border-gray-300',
        outline: 'border border-[#FDB4B7] text-[#FDB4B7] hover:bg-[#FDB4B7] hover:text-white bg-transparent',
    ...
    // Renderizado del icono más robusto
    const renderIcon = () => {
        if (!icon) return null;

        // Si es un componente React (JSX)
        if (React.isValidElement(icon)) {
            return icon;
        }

        // Si es una string (URL de imagen)
