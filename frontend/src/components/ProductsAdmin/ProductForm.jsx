...
const ProductForm = ({
  ...
  useEffect(() => {
    if (isOpen) {
      // Si productData existe (editando), usar sus valores, sino usar valores por defecto
      if (productData) {
        ...
        // Si no hay productData (creando nuevo), resetear a valores por defecto
        setValue('name', '');
            ...
            <Controller
              ...
              render={({ field }) => (
                <textarea
                  {...field}
                  rows="2"
                  className="w-full px-4 py-2 border rounded-lg border-gray-300 focus:ring-2 focus:ring-[#FF7260]"
