/**
 * Context de autenticación para la aplicación - CORRECCIÓN CRÍTICA PARA SOCKET.IO
 * 
 * CAMBIOS IMPLEMENTADOS:
 * - Sincronización completa antes de marcar como autenticado
 * - Mejor gestión de tokens híbridos para Socket.IO
 * - Delay estratégico para asegurar que tokens estén disponibles
 * - Verificación de token antes de establecer estado autenticado
 * 
 * ✅ CORREGIDO: Headers de autenticación híbridos y sincronización con Socket.IO
 */
...
export const AuthProvider = ({ children }) => {
    ...
    const [attemptsStorage, setAttemptsStorage] = useState({});
    const [lockoutInfo, setLockoutInfo] = useState(null);
    const [userOrderStats, setUserOrderStats] = useState(null);

    // ✅ NUEVO: Estado para token en memoria (verificación híbrida)
    const [authToken, setAuthToken] = useState(null);
    const [tokenSyncComplete, setTokenSyncComplete] = useState(false); // ✅ CRÍTICO PARA SOCKET.IO

    /**
     * ✅ MEJORADO: Obtiene el token de cookies con debug mejorado
     */
    ...
    /**
     * ✅ ACTUALIZADO: Obtiene el mejor token disponible (verificación híbrida)
     * CRÍTICO PARA SOCKET.IO: Prioriza URL > localStorage > cookies > memoria
     */
    const getBestAvailableToken = () => {
        console.log('🔍 === BÚSQUEDA DE MEJOR TOKEN DISPONIBLE ===');
        ...
        if (urlToken) {
            ...
            if (decoded) {
                ...
                setAuthToken(urlToken); // ✅ SINCRONIZAR EN MEMORIA
            }
            return urlToken;
        }
        ...
        if (localToken) {
            ...
            setAuthToken(localToken); // ✅ SINCRONIZAR EN MEMORIA
            return localToken;
        }
        ...
        if (cookieToken) {
            ...
            if (decoded) {
                ...
                setAuthToken(cookieToken); // ✅ SINCRONIZAR EN MEMORIA
            }
            return cookieToken;
        }
    ...
    /**
     * ✅ CORRECCIÓN CRÍTICA: Headers de autenticación híbridos
     */
    ...
    /**
     * ✅ CORRECCIÓN CRÍTICA: getUserOrderStats con headers híbridos
     */
    ...
    /**
     * ✅ CORRECCIÓN CRÍTICA: getUserInfo con headers híbridos
     */
    ...
    const clearAuthData = (isVoluntaryLogout = false) => {
        ...
        try {
            ...
            // ✅ CRÍTICO: Limpiar estado de memoria también
            setAuthToken(null);
            setTokenSyncComplete(false); // ✅ RESET SINCRONIZACIÓN
            setUser(null);
    ...
    /**
     * ✅ CORRECCIÓN CRÍTICA: checkAuthStatus con sincronización mejorada para Socket.IO
     */
    const checkAuthStatus = async () => {
        try {
            console.log('🔍 === INICIO checkAuthStatus MEJORADO ===');
            ...
            // ✅ PASO 1: Buscar token de todas las fuentes
            ...
            // ✅ PASO 3: Asegurar persistencia en localStorage
            ...
            // ✅ PASO 4: Establecer estado de usuario (CRÍTICO PARA SOCKET.IO)
            const userState = {
                id: decodedToken.id,
                userType: decodedToken.userType
            };
            ...
            // ✅ PASO 5 CRÍTICO: Dar tiempo para sincronización completa
            console.log('⏳ Esperando sincronización completa de token...');
            
            // ✅ Delay estratégico para asegurar que Socket.IO tenga acceso al token
            await new Promise(resolve => setTimeout(resolve, 300));
            
            // ✅ MARCAR SINCRONIZACIÓN COMPLETA
            setTokenSyncComplete(true);
            console.log('✅ Token completamente sincronizado para Socket.IO');
            
            // ✅ AHORA SÍ marcar como autenticado (después de sincronización)
            setIsAuthenticated(true);
            
            // ✅ PASO 6: Obtener información completa en background
            try {
            ...
            setLoading(false);
            console.log('🎉 checkAuthStatus completado exitosamente con sincronización');
            ...
            console.log('🔍 === FIN checkAuthStatus MEJORADO ===');
        }
    };
    ...
    /**
     * ✅ CORRECCIÓN CRÍTICA: login con sincronización mejorada
     */
    const login = async (email, password) => {
        try {
            ...
            if (data.success && data.token) {
                console.log('✅ Login exitoso, procesando token con sincronización...');
                ...
                // ✅ GUARDAR EN MÚLTIPLES LUGARES CON SINCRONIZACIÓN
                ...
                if (decodedToken) {
                    ...
                    // ✅ PASO CRÍTICO: Establecer usuario PERO esperar para marcar como autenticado
                    setUser(userState);
                    setAuthError(null);
                    setLockoutInfo(null);

                    // ✅ SINCRONIZACIÓN CRÍTICA: Esperar a que el token esté completamente disponible
                    console.log('⏳ Esperando sincronización completa del token...');
                    await new Promise(resolve => setTimeout(resolve, 500)); // ✅ Tiempo para sincronización

                    // ✅ MARCAR SINCRONIZACIÓN COMPLETA
                    setTokenSyncComplete(true);
                    console.log('✅ Token completamente sincronizado');

                    // ✅ AHORA SÍ marcar como autenticado (después de sincronización)
                    setIsAuthenticated(true);

                    // Obtener información completa en background
                    try {
                        await getUserInfo();
                    } catch (userInfoError) {
                    ...
                    console.log('🎉 Login completado exitosamente con sincronización garantizada');
    ...
    /**
     * ✅ CORRECCIÓN CRÍTICA: logout con headers híbridos
     */
    const logout = async () => {
        try {
    ...
    /**
     * Limpia intentos de login manualmente
     */
    ...
    // ✅ EFECTO PRINCIPAL: Verificación inicial con detección de URL token
    useEffect(() => {
    ...
    // ✅ EFECTO: Verificación periódica de sesión
    useEffect(() => {
        let sessionRefreshInterval;

        if (isAuthenticated && !isLoggingOut && !isLoggingIn && tokenSyncComplete) {
    ...
    }, [isAuthenticated, isLoggingOut, isLoggingIn, tokenSyncComplete]);

    // ✅ EFECTO: Detectar cuando la página vuelve a estar visible
    useEffect(() => {
        const handleVisibilityChange = () => {
            if (document.visibilityState === 'visible' && isAuthenticated && !isLoggingOut && !isLoggingIn && tokenSyncComplete) {
    ...
    // ✅ EFECTO: Detectar cierre de pestaña/navegador
    useEffect(() => {
    ...
    // ✅ EFECTO: Verificación periódica inteligente
    useEffect(() => {
    ...
    const contextValue = {
        ...
        userOrderStats,
        authToken,
        tokenSyncComplete, // ✅ NUEVO: Estado crítico para Socket.IO
        ...
        // ✅ Funciones críticas para Socket.IO
        setAuthToken,
