...
const SalesManager = () => {
    ...
    useEffect(() => {
        if (sales.length > 0) {
            const agendadas = sales.filter(sale => sale.trackingStatus === 'Agendado').length;
            const enProceso = sales.filter(sale => sale.trackingStatus === 'En proceso').length;
            const entregadas = sales.filter(sale => sale.trackingStatus === 'Entregado').length;
    ...
    if (loading) {
        return (
            <div className="min-h-screen bg-gray-50">
                <NavbarAdmin />
                <div className="ml-16 p-6 flex justify-center items-center">
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            ...
            {/* Contenido principal */}
            <div className="ml-0 md:ml-16 p-3 sm:p-4 lg:p-6">
                {/* Header */}
                ...
                <div className="mt-8 bg-white rounded-lg shadow-sm p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="flex items-center gap-3">
                            <div className={`px-3 py-1 rounded-full text-xs font-medium border ${getStatusColor('Agendado')}`}>
                                Agendado
                            </div>
                            <span className="text-sm text-gray-600">Pedido confirmado, esperando preparación</span>
                        </div>
                        <div className="flex items-center gap-3">
                            <div className={`px-3 py-1 rounded-full text-xs font-medium border ${getStatusColor('En proceso')}`}>
                                En proceso
                            </div>
                            <span className="text-sm text-gray-600">Pedido en preparación</span>
                        </div>
                        <div className="flex items-center gap-3">
                            <div className={`px-3 py-1 rounded-full text-xs font-medium border ${getStatusColor('Entregado')}`}>
                                Entregado
                            </div>
